...
class CompleteServer:
    ...
    def _run_scheduled_task(self, task_id):
        ...
        if delay > 0:
            # Спим небольшими интервалами, чтобы вовремя заметить отмену/удаление
            sleep_until = time.time() + delay
            while time.time() < sleep_until:
                time.sleep(min(0.5, sleep_until - time.time()))
                with self.scheduler_lock:
                    current = self.scheduled_tasks.get(task_id)
        ...
        if task.get('repeat_interval', 0) > 0:
            with self.scheduler_lock:
                current = self.scheduled_tasks.get(task_id)
                if current and current['status'] not in ('cancelled', 'deleted'):
    ...
    def cancel_task(self, task_id):
        """Отменить задачу (статус → cancelled, остаётся в списке)"""
        with self.scheduler_lock:
            if task_id in self.scheduled_tasks:
    ...
    def delete_task(self, task_id):
        """Удалить задачу из списка полностью"""
        with self.scheduler_lock:
            if task_id in self.scheduled_tasks:
                self.scheduled_tasks[task_id]['status'] = 'deleted'
    ...
    def clear_command_history(self):
        ...
        self.log("История команд очищена")
        return {'type': 'clear_history', 'success': True}
    ...
    def capture_microphone(self, duration=5, sample_rate=44100, channels=1, device_index=None):
        """Запись звука с микрофона"""
        if not HAS_PYAUDIO:
            return {
                'type': 'microphone',
                'error': 'pyaudio не установлен. Установите: pip install pyaudio',
                'success': False
            }
        try:
            dev_str = f" (устройство {device_index})" if device_index is not None else ""
            self.log(f"Запись микрофона {duration} сек{dev_str}...")
            pa = pyaudio.PyAudio()
            open_kwargs = dict(
            ...
            # Упаковываем в WAV через io.BytesIO
            wav_buf = io.BytesIO()
            import wave
            ...
            self.log(f"Запись завершена, размер: {len(wav_bytes)} байт")
            return {
                'type': 'microphone',
                'data': encoded,
                'format': 'wav',
            ...
            error_msg = f"Ошибка записи микрофона: {str(e)}"
            self.log(f"Ошибка: {error_msg}")
            return {'type': 'microphone', 'error': error_msg, 'success': False}

    def capture_mic_chunk(self, sample_rate=16000, channels=1, chunk_ms=200, device_index=None):
        """Захватить короткий чанк PCM с микрофона для потоковой трансляции"""
        if not HAS_PYAUDIO:
            return {
                'type': 'mic_chunk',
                'error': 'pyaudio не установлен. Установите: pip install pyaudio',
        ...
        try:
            ...
            return {'type': 'mic_chunk', 'error': str(e), 'success': False}
    ...
    def list_audio_devices(self):
        ...
        try:
            ...
            pa.terminate()
            return {'type': 'audio_devices', 'devices': devices, 'success': True}
        except Exception as e:
            return {'type': 'audio_devices', 'error': str(e), 'success': False}

    # ─────────────── КАМЕРА ───────────────

    def capture_camera_frame(self, camera_index=0, width=640, height=480, quality=70):
        """Захват кадра с камеры"""
        if not HAS_CV2:
            return {
                'type': 'camera_frame',
                'error': 'opencv-python не установлен. Установите: pip install opencv-python',
                'success': False
            }
        cap = None
        try:
            self.log(f"Захват кадра с камеры {camera_index}...")
            cap = cv2.VideoCapture(camera_index)
            if not cap.isOpened():
                return {'type': 'camera_frame', 'error': f'Камера {camera_index} недоступна', 'success': False}
            cap.set(cv2.CAP_PROP_FRAME_WIDTH, width)
            cap.set(cv2.CAP_PROP_FRAME_HEIGHT, height)
            # Несколько итераций для прогрева камеры
            for _ in range(3):
                ret, frame = cap.read()
            if not ret or frame is None:
                return {'type': 'camera_frame', 'error': 'Не удалось захватить кадр', 'success': False}
            ret2, jpeg = cv2.imencode('.jpg', frame, [cv2.IMWRITE_JPEG_QUALITY, quality])
            if not ret2:
                return {'type': 'camera_frame', 'error': 'Ошибка кодирования JPEG', 'success': False}
            encoded = base64.b64encode(jpeg.tobytes()).decode('utf-8')
            h, w = frame.shape[:2]
            self.log(f"Кадр захвачен: {w}x{h}")
            return {
                'type': 'camera_frame',
                'data': encoded,
                'format': 'jpeg',
            ...
            error_msg = f"Ошибка захвата камеры: {str(e)}"
            self.log(f"Ошибка: {error_msg}")
            return {'type': 'camera_frame', 'error': error_msg, 'success': False}
        finally:
            if cap:
                cap.release()
    ...
    def list_cameras(self):
        ...
        if not HAS_CV2:
            return {
                'type': 'camera_list',
                'error': 'opencv-python не установлен',
                'success': False
            }
        cameras = []
        for i in range(5):
            ...
            if cap.isOpened():
                ...
                cameras.append({'index': i, 'name': f'Камера {i}', 'width': w, 'height': h})
                cap.release()
        return {'type': 'camera_list', 'cameras': cameras, 'success': True}
    ...
    def process_get_processes(self):
        ...
        attrs = ['pid', 'name', 'status', 'username', 'memory_info',
                 'memory_percent', 'cpu_percent', 'create_time',
                 'num_threads', 'exe', 'ppid']

        # Первый вызов cpu_percent инициализирует счётчики — делаем заранее
        ...
        time.sleep(0.3)  # Короткая пауза для накопления CPU-статистики
        ...
        for p in psutil.process_iter(attrs):
            try:
                ...
                # CPU на один логический процессор
                raw_cpu = p.cpu_percent(interval=None) or 0.0
                cpu = round(raw_cpu / total_cpu, 2)

                # Время создания процесса
                try:
                    ct = info.get('create_time') or 0
                    create_str = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(ct)) if ct else '—'
                except Exception:
                    create_str = '—'
                ...
                processes.append({
                    ...
                    "cpu":        f"{cpu:.1f}%",
                    "memory_mb":  f"{rss // (1024 * 1024)} MB",
                    "memory_pct": f"{round(info.get('memory_percent') or 0, 1)}%",
                    "vms_mb":     f"{vms // (1024 * 1024)} MB",
        ...
        try:
            ...
            if command_type == 'command':
                ...
                return {
                    ...
                    'task_id': task_id,
                    'success': cancelled,
                    'error': None if cancelled else 'Задача не найдена или уже выполнена'
                }
                ...
                return {
                    ...
                    'task_id': task_id,
                    'success': deleted,
                    'error': None if deleted else 'Задача не найдена'
                }

            elif command_type == 'clear_history':
                return self.clear_command_history()
                ...
                device_index = request.get('device_index', None)
                if device_index is not None:
                    device_index = int(device_index)
                return self.capture_microphone(duration, sample_rate, channels, device_index)

            elif command_type == 'mic_chunk':
                sample_rate = int(request.get('sample_rate', 16000))
                channels = int(request.get('channels', 1))
                chunk_ms = int(request.get('chunk_ms', 200))
                device_index = request.get('device_index', None)
                if device_index is not None:
                    device_index = int(device_index)
                return self.capture_mic_chunk(sample_rate, channels, chunk_ms, device_index)
                ...
                width = int(request.get('width', 640))
                height = int(request.get('height', 480))
                quality = int(request.get('quality', 70))
                return self.capture_camera_frame(camera_index, width, height, quality)
